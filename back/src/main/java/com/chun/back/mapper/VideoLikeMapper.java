...
public interface VideoLikeMapper {
    ...
    @Select("SELECT COUNT(*) FROM video_like WHERE video_id=#{videoId} AND user_id=#{userId}")
    int exists(Long videoId, Long userId);

    @Insert("INSERT IGNORE INTO video_like(video_id, user_id, create_time) VALUES(#{videoId}, #{userId}, NOW())")
